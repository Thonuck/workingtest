...
class Starter(db.Model):
    ...
    dog_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("dog.id"), nullable=True
    )
    ...
    person: Mapped[Optional[Person]] = relationship(
        "Person", foreign_keys=[person_id], backref=db.backref("starters", lazy=True)
    )
    dog: Mapped[Optional[Dog]] = relationship(
        "Dog", foreign_keys=[dog_id], backref=db.backref("starters", lazy=True)
    )
    competition: Mapped[Optional[Competition]] = relationship(
        "Competition",
        foreign_keys=[competition_id],
        backref=db.backref("starters", lazy=True),
    )
...
class Exercise(db.Model):
    ...
    competition_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("competition.id"), nullable=False
    )
    ...
    max_points: Mapped[int] = mapped_column(Integer, default=20, nullable=False)

    # Relationships
    competition: Mapped[Competition] = relationship(
        "Competition", backref=db.backref("exercises", lazy=True)
    )
    judge: Mapped[Optional[Person]] = relationship(
        "Person",
        foreign_keys="Exercise.judge_id",
        backref=db.backref("judged_exercises", lazy=True),
    )
    helper: Mapped[Optional[Person]] = relationship(
        "Person",
        foreign_keys="Exercise.helper_id",
        backref=db.backref("helped_exercises", lazy=True),
    )
...
class ExercisePointEntry(db.Model):
    ...
    starter_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("starter.id"), nullable=False
    )
    ...
    exercise: Mapped[Exercise] = relationship(
        "Exercise", backref=db.backref("point_entries", lazy=True)
    )
    starter: Mapped[Starter] = relationship(
        "Starter", backref=db.backref("point_entries", lazy=True)
    )
...
class ExerciseResult(db.Model):
    ...
    # Relationships
    exercise: Mapped[Exercise] = relationship(
        "Exercise", backref=db.backref("results", lazy=True)
    )
    starter: Mapped[Starter] = relationship(
        "Starter", backref=db.backref("exercise_results", lazy=True)
    )
...
class CompetitionResult(db.Model):
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    competition_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("competition.id"), nullable=False, unique=True
    )
    published: Mapped[bool] = mapped_column(Boolean, default=False)
    published_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True
    )  # Set when published=True

    # Relationship
    competition: Mapped[Competition] = relationship(
        "Competition", backref=db.backref("result_status", uselist=False)
    )
