...
# ==================== WT EXERCISES PAGE ====================
@bp.route("/wt/<int:competition_id>", methods=["GET", "POST"])
@login_required
...
def wt_exercises(competition_id):
    ...
    exercise_db = Exercise.query.get(competition_id)
        ...
        exercises = [
            {
                "id": exercise.id,
                "name": exercise.name,
                "judge": exercise.judge.name,
                "helper": exercise.helper.name,
            }
    ...
    table_data: dict[str, Any] = {
        "title": "Aufgaben",
        "headers": [("name", "Aufgabe"), ("judge", "Richter"), ("helper", "Helfer")],
        "items": exercises,
        "details_route": "users.detail",
    }

    return render_template("wt_exercises.html.jinja", table_data=table_data)
...
def add_exercise(competition_id):
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("exercises.wt_exercises", competition_id=competition_id)
        )

    users = User.query.all()
    return render_template(
        "add_exercise.html.jinja", competition=competition, users=users
    )
...
def edit_exercise(exercise_id):
    ...
    if request.method == "POST":
        ...
        exercise.max_points = request.form.get(
            "max_points", exercise.max_points, type=int
        )
        ...
        return redirect(
            url_for("exercises.wt_exercises", competition_id=exercise.competition_id)
        )
...
def exercise_point_entry(exercise_id):
    ...
    if not (
        ...
        or current_user.id == exercise.helper_id
    ):
        abort(403)
    ...
    if request.method == "POST":
            ...
            if points_input:
                try:
                    ...
                    entry = ExercisePointEntry.query.filter_by(
                        exercise_id=exercise_id, starter_id=starter.id
                    ).first()
        ...
        return redirect(
            url_for("exercises.exercise_point_entry", exercise_id=exercise_id)
        )
...
def competition_results(competition_id):
    ...
    is_admin_or_organizer = current_user.is_authenticated and current_user.role in [
        "admin",
        "organizer",
    ]
        ...
        for starter in starters:
            ...
            for exercise in exercises:
                point_entry = ExercisePointEntry.query.filter_by(
                    exercise_id=exercise.id, starter_id=starter.id
                ).first()
...
def publish_results(competition_id):
    ...
    flash(
        "Results published successfully. Visitors can now view the results.", "success"
    )
    return redirect(
        url_for("exercises.competition_results", competition_id=competition_id)
    )
